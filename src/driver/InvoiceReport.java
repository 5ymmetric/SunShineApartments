...
public class InvoiceReport {
	...
	public static void ConsoleWriter(Set<Invoice> Invoices) {
		...
		for (Invoice i : Invoices) { // Loops through all the Invoices
			...
			// print statements for each invoice
			System.out.println(String.format("%-10s %-42s %-25s  %20.2f $ %10.2f $ %15.2f $ %10.2f $ %15.2f $",
					i.getInvoiceCode(), i.getCustomer().getName() + " " + i.getCustomer().getModifiedType(),
					i.getRealtor().getFullName(), subtotal, fees, taxes, totalDiscount, total)); // Certain string
			// formatting
		}
		...
		for (Invoice i : Invoices) { // Loop for iterating and printing detailed reports
			...
			System.out.println("\n				Thank you for your purchase! "); // Thank you statement!
		}
